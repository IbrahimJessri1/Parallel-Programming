...
public final class StudentAnalytics {
    ...
    public double averageAgeOfEnrolledStudentsParallelStream(
            final Student[] studentArray) {
        OptionalDouble res =  Arrays.stream(studentArray).parallel().filter(x -> x.checkIsCurrent()).mapToDouble(x -> x.getAge()).average();
        if(res.isPresent())
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
            final Student[] studentArray) {
        String res =  Arrays.stream(studentArray)
                    .parallel()
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
            final Student[] studentArray) {
        return (int)Arrays.stream(studentArray).parallel().filter(x -> (!x.checkIsCurrent() && x.getAge() > 20 && x.getGrade() < 65)).count();
    }
}
